// Package file provides file-backed metric snapshot storage.
package file
...
import (
	...
	"github.com/iPatrushevSergey/metrics/metrics_new/app/internal/server/metrics/adapters/repository/file/converter"
	"github.com/iPatrushevSergey/metrics/metrics_new/app/internal/server/metrics/adapters/repository/file/model"
...
func (r *MetricFileRepository) LoadAll(_ context.Context) ([]entity.Metric, error) {
	file, err := os.OpenFile(r.filePath, os.O_RDONLY|os.O_CREATE, 0o666)
	if err != nil {
		return nil, err
	}
	defer file.Close()
