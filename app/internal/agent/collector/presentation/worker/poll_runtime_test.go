...
func TestPollRuntimeWorker_Run(t *testing.T) {
	...
	factory := stubUseCaseFactory{
		pollRuntime: stubUseCase{onExecute: func(context.Context, struct{}) (int, error) {
			ticks.Add(1)
			return 1, nil
		}},
	}

	w := NewPollRuntimeWorker(factory, log, time.Millisecond)
